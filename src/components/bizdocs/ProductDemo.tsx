...
const rawFiles = [{
  name: 'Q3_invoices.pdf',
  type: 'Revenue',
  color: 'bg-green-500'
}, {
  name: 'payroll_sept.xlsx',
  type: 'Payroll',
  color: 'bg-blue-500'
}, {
    ...
    <div className="space-y-6">
      ...
      {uploadedFiles.length > 0 && <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {uploadedFiles.map((file, index) => <div key={index} className="p-4 bg-card border rounded-lg group hover:shadow-md transition-all">
                <div className="flex items-start justify-between mb-2">
                  <FileText className="w-6 h-6 text-muted-foreground" />
                  <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100 h-6 w-6 p-0" onClick={e => {
              e.stopPropagation();
              removeFile(index);
            }}>
...
const CleaningStep = ({
  ...
  const saveEdit = (id: number) => {
    setCleanedData(prev => prev.map(item => item.id === id ? {
      ...item,
      cleaned: editValue
    } : item));
    setEditingId(null);
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={autoClean} disabled={isProcessing} variant="outline" size="sm">
            {isProcessing ? <>
                <Sparkles className="w-4 h-4 mr-2 animate-spin" />
                Cleaning...
              </> : <>
        ...
        <div className="border rounded-lg overflow-hidden">
          ...
          {cleanedData.map(row => <div key={row.id} className="grid grid-cols-5 gap-4 p-3 border-t hover:bg-muted/50">
              ...
              <div className="flex items-center gap-2">
                {editingId === row.id ? <Input value={editValue} onChange={e => setEditValue(e.target.value)} className="h-6 text-xs" onKeyDown={e => e.key === 'Enter' && saveEdit(row.id)} autoFocus /> : <span className="text-sm font-medium text-green-600">{row.cleaned}</span>}
              </div>
              <div className="flex gap-1">
                {editingId === row.id ? <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => saveEdit(row.id)}>
                    <CheckCircle className="w-3 h-3" />
                  </Button> : <Button size="sm" variant="ghost" className="h-6 w-6 p-0" onClick={() => startEdit(row.id, row.cleaned)}>
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="md:col-span-2 space-y-4">
          ...
          {showAddCategory && <div className="flex gap-2 p-3 bg-muted/50 rounded-lg">
              <Input placeholder="Category name" value={newCategoryName} onChange={e => setNewCategoryName(e.target.value)} onKeyDown={e => e.key === 'Enter' && addCategory()} className="flex-1" />
              <Button size="sm" onClick={addCategory}>Add</Button>
            </div>}

          <div className="space-y-3">
            {categories.map(category => <div key={category.id} className="flex items-center justify-between p-4 bg-card border rounded-lg group">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Button variant="ghost" size="sm" className="opacity-0 group-hover:opacity-100 h-6 w-6 p-0" onClick={() => removeCategory(category.id)}>
                    <Trash2 className="w-3 h-3" />
                  </Button>
                </div>
              </div>)}
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="space-y-3">
          ...
          <Slider value={rentAdjustment} onValueChange={setRentAdjustment} max={50} min={-30} step={5} className="w-full" />
        ...
        <div className="space-y-3">
          ...
          <Slider value={marketingBudget} onValueChange={setMarketingBudget} max={100} min={-50} step={5} className="w-full" />
          <p className="text-xs text-muted-foreground">
        ...
        <div className="space-y-3">
          ...
          <Slider value={revenueGrowth} onValueChange={setRevenueGrowth} max={100} min={-20} step={5} className="w-full" />
          <p className="text-xs text-muted-foreground">
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          return <div key={format.id} className={`p-4 border rounded-lg cursor-pointer transition-all ${selectedFormat === format.id ? 'border-primary bg-primary/5' : 'border-muted hover:border-muted-foreground/40'}`} onClick={() => setSelectedFormat(format.id)}>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <label className="flex items-center space-x-2 cursor-pointer">
            <input type="checkbox" checked={includeProjections} onChange={e => setIncludeProjections(e.target.checked)} className="rounded" />
      ...
      {/* Export Result */}
      {exported && <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
